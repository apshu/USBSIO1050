...
#include "fileio/static_filesys.h"
...
const uint8_t VBR_seg0[] = {
    ...
    /* 0x00d */ DRV_SECTORS_PER_CLUSTER, // Sectors per cluster (16*512=8kB)
...
void VolumeBootRecordGet(uint8_t * buffer, uint8_t seg) { // fabricate an MBR structure in the RAM buffer
    ...
    if (0 == seg) { // segment from 000 to 0x03f
        ...
        memcpy((void*) &buffer[ 0x02b], (void*) "EEP_SIO2USB", 11);
        // FAT system ( 8 bytes)
...
static uint_fast8_t FATclusterByte(uint_fast16_t cluster1, uint_fast16_t cluster2, uint_fast8_t byteID) {
    cluster1 &= 0xFFF; /* Limit to 12bit - FAT12 */
    cluster2 &= 0xFFF; /* Limit to 12bit - FAT12 */
    switch (byteID % 3) {
        case 0:
            return cluster1 & 0xFF;
        case 1:
            return ((cluster1 >> 8)& 0x0F) | ((cluster2 & 0x0F) << 4);
        case 2:
...
void FATRecordGet(uint8_t * buffer, uint8_t seg) {
    ...
    if (seg == 0) {
        ...
        buffer[4] = 0x4F; // DRIVER.HTM, EEPIMAGE.ATR   //First Cluster in Chain is 3
        buffer[5] = 0x00; // EEPIMAGE.ATR
        i = 6;
        uint_fast16_t clusterChain;
        for (clusterChain = 1; (i < 64) && (clusterChain < ((EEPROM_BYTE_SIZE / DRV_SECTORS_PER_CLUSTER / FILEIO_CONFIG_MEDIA_SECTOR_SIZE) - 0)); clusterChain += 2) {
            uint_fast16_t nextCluster;
            if (clusterChain == ((EEPROM_BYTE_SIZE / DRV_SECTORS_PER_CLUSTER / FILEIO_CONFIG_MEDIA_SECTOR_SIZE) - 1)) {
                /* Last Cluster */
                clusterChain = 0xFFF - 4;
                nextCluster = 0;
            } else {
                if (clusterChain == ((EEPROM_BYTE_SIZE / DRV_SECTORS_PER_CLUSTER / FILEIO_CONFIG_MEDIA_SECTOR_SIZE) - 2)) {
                    nextCluster = 0xFFF;
                } else {
                    nextCluster = clusterChain + 4 + 1;
                }
            }

            buffer[i++] = FATclusterByte(clusterChain + 4, nextCluster, i); // EEPIMAGE.ATR
            if (i < 64) {
                buffer[i++] = FATclusterByte(clusterChain + 4, nextCluster, i); // EEPIMAGE.ATR
                if (i < 64) {
                    buffer[i++] = FATclusterByte(clusterChain + 4, nextCluster, i); // EEPIMAGE.ATR
...
const char readme[] = "<html><head><meta http-equiv=\"refresh\" content=\"0;URL='http://microchip.com/mcp2221a'\" /></head></html>";

uint8_t readme_size(void) {
    return (sizeof ( readme) - 1); // required by direct.c (external references return always size 0!?)
}

const uint8_t entry0[ ROOT_ENTRY_SIZE] = {
    // Drive Name (11 characters, padded with spaces)
    'E', 'E', 'P', '_', 'S', 'I', 'O', '2', 'U', 'S', 'B',
...
const uint8_t entry1[ ROOT_ENTRY_SIZE] = {
    'D', 'R', 'I', 'V', 'E', 'R', ' ', ' ', // File name (exactly 8 characters)
    ...
    sizeof (readme) - 1, 0x00, 0x00, 0x00, // README string size (<256)
};

const uint8_t entry2[ ROOT_ENTRY_SIZE] = {
    'E', 'E', 'P', 'I', 'M', 'A', 'G', 'E', // File name (exactly 8 characters)
    'A', 'T', 'R', // File extension (exactly 3 characters)
    0x20, // specify this entry as a regular file
    0x00, // Reserved
    0x00, // Creation time, fine res 10 ms units (0-199)
    TIMEL(MAJOR, MINOR, 0), // Creation time, hour/min/sec
    ...
    0x03, 0x00, // First FAT cluster (#3 is available)
    EEPROM_BYTE_SIZE & 0xFF, (EEPROM_BYTE_SIZE >> 8) & 0xFF, (EEPROM_BYTE_SIZE >> 16) & 0xFF, (EEPROM_BYTE_SIZE >> 24) & 0xFF, // EEPROM image size 
};
